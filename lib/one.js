...
exports.init = function (gulp, options) {
    ...
    one.transforms = {
        ...
        concatJs: js => {
            ...
            var final = _(one.options.jsDeps).chain()
                .map(dep => {
    ...
    gulp.task('watch', function () {
        ...
        watcher.watch(one.options.src, {
            ...
            unlink(filePath) {
                ...
                var fileToDelete = srcToDevMapping[path.resolve(filePath)];

                fs.existsSync(fileToDelete, function (exists) {
                    if (fileToDelete) {
                        rimraf.sync(fileToDelete);
                    }
                });
