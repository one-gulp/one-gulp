...
exports.init = function (gulp, options) {
    ...
    gulp.task('graph', function (done) {
        var fileRelativePath = './one-gulp-streams-graph.svg';
        var fileAbsolutePath = path.join(process.cwd(), fileRelativePath);
        loggers.notifier('Writing SVG file at ' + fileAbsolutePath);

        one.renderGraph('dot', fileRelativePath, done);
    });
};
