...
exports.init = function (gulp, options) {
    ...
    one.sources = {
        ...
        json: () => src.fromExts(one.options.src, 'json'),
        ...
        other: () => src.notFromExts(one.options.src, ['html', 'css', 'js', 'json', 'gif', 'jpg', 'jpeg', 'png', 'svg'])
    };
    ...
    one.link(one.sources.json).to(one.transforms.minifyJson);
    one.link(one.transforms.minifyJson).to(one.outputs.writeToProd);
    ...
    gulp.task('watch', function () {
        ...
        var watchPaths = _([[options.src], options.watchPaths]).chain()
            .flatten()
            .compact()
            .value();

        watcher.watch(watchPaths, debouncedBrowserSync);
    });
