...
exports.browserSyncReload = function (paths) {
    ...
    var firstPath = chalk.magenta(paths[0]),
        otherPaths = paths.slice(1);

    console.log(getColoredTime(), 'reload:', chalk.blue.bold('BrowserSync'), 'on', firstPath);

    otherPaths.forEach(function (path) {
        console.log(getColoredTime(), new Array('reload: BrowserSync on '.length).join(' '), chalk.magenta(path));
    });
};
